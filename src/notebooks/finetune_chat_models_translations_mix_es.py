# -*- coding: utf-8 -*-
# This script fine-tunes an OpenAI model for Mixtec-to-Spanish translation.
...
def upload_file(file_name: str, purpose: str) -> str:
    with open(file_name, "rb") as file_fd:
        response = client.files.create(file=file_fd, purpose=purpose)  # Uploads file
    return response.id  # Returns the file ID

# Define file names for training and validation datasets
training_file_name = r"train_mixtec_espanol.jsonl"
validation_file_name = r"val_mixtec_espanol.jsonl"

# Upload the training and validation files and retrieve their IDs
...
# Define the model to be fine-tuned and experiment details
MODEL = "gpt-4o-mini-2024-07-18" 
PROJECT_NAME = "Finetuning_mixtec_translator"
EXPERIMENT_NAME = f"{MODEL}-GPT_MIXTEC_SPANISH_05_03"
...
response = client.fine_tuning.jobs.create(
    ...
    model=MODEL,  # Base model for fine-tuning
    suffix=EXPERIMENT_NAME,  # Name for the fine-tuned model
    integrations=[
        {
...
print("Job ID:", response.id)
print("Status:", response.status)
print("Trained Tokens:", response.trained_tokens)  # Number of processed tokens
